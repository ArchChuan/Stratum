...
// ResolveDefaultTenantID returns the real id of the default tenant row
// (tenants.id is a UUID generated by uuid_generate_v4(), never the literal
// "tenant_default"). Management-plane gates derive the default tenant id from
// this instead of comparing against the literal. Missing or deleted row fails
// closed (startup error) so gates never silently compare against a fabricated id.
func ResolveDefaultTenantID(ctx context.Context, queryer readinessQueryer) (string, error) {
	var id string
	if err := queryer.QueryRow(ctx,
		`SELECT id FROM public.tenants WHERE is_default = true AND deleted_at IS NULL LIMIT 1`,
	).Scan(&id); err != nil {
		return "", fmt.Errorf("postgres: resolve default tenant id: %w", err)
	}
	return id, nil
}
