...
func (c *Container) buildMemoryService(mem *Memory, db *pgxpool.Pool, memRepo memport.MemoryRepo) {
	...
	if c.LLMGateway != nil && c.LLMGateway.Registry != nil {
		...
		mem.Service.SetLLMExtractResolver(makeLLMExtractResolver(llmRes, c.memoryResourceParamResolver(), baseline, c.Logger))
...
func makeLLMExtractResolver(llmRes *tenantCapabilityResolver, resolver memport.ResourceParamResolver, baseline memport.MechanismBaselineResolver, logger *zap.Logger) func(context.Context, string) memport.LLMExtractor {
