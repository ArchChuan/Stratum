...
本 point（`retrieval`）的基线文件为 `retrieval.json`（由 `points/retrieval.yaml` 的 `baseline: ../baselines/retrieval.json` 指向）。注意：目录中的 `latest.json` 是历史遗留基线，属历史产物，与本单点评测无关，不要修改或复用。
