...
`.go-arch-lint.yml` 与 `.golangci.yml` 中的 depguard 是规则来源。当前 GitHub CI 的 lint job 运行 golangci-lint（因此执行 depguard），但没有单独执行 `go-arch-lint`；本地修改包边界时应额外运行 `go-arch-lint`。
...
| `handler/` | bind → 取 tenant → call service → render，错误用 `c.Error(err)` 交给 ErrorHandler | import `pgx*`/`redis*`/`milvus*`/`internal/*/infrastructure`；散写白名单/SQL/编排 |
| `middleware/` | 横切：auth · trace · metrics · `domain.Err* → HTTP` 映射 | 任何业务编排 |
| `wiring/` | 组合根：构造 application + infrastructure，塞 Container，反向 Shutdown；连接跨 context adapter | 处理 HTTP/业务规则 |
...
| 决策 | 原因 |
|------|------|
| 多租户 PostgreSQL schema 隔离 | tenant-scoped repository 通过 `pkg/tenantdb` 在事务内设置 `search_path` |
| JWT RS256 | `api/middleware/jwt.go` 验证 RSA 签名并注入 claims |
| NATS JetStream | 当前用于 Memory pipeline 的持久化异步处理 |
| Milvus v2.4.2 | Knowledge 与 Memory 的向量存储实现 |
| Harness 生命周期管理 | runtime 注册后台组件并统一启动、停止 |
| LLMGateway 统一抽象 | 对 Agent、Memory、Knowledge 暴露统一 completion/embed 能力 |
| No AI control logic | 路由、权限、重试和状态机由代码控制，模型只在允许的工具与指令范围内决策 |
