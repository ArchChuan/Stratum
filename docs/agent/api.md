...
| PATCH | `/skills/:id/draft/activation` | UpdateDraftActivation | admin + requireActive |
| PATCH | `/skills/:id/draft/instructions` | UpdateDraftInstructionBundle | admin + requireActive |
...
MCP 路由在 `api/http/handler/mcp_handler.go` 的 `RegisterRoutes` 中定义。所有路由至少需要 member；服务器连接、更新、断开、删除配置、重连以及工具风险策略修改需要 admin + requireActive。当前没有通用 HTTP 工具执行路由，Agent 通过内部 `MCPToolExecutor` 调用工具。

主要路径：`/mcp/servers`、`/mcp/servers/:id`、`/mcp/servers/:id/tools`、`/mcp/servers/:id/resources`、`/mcp/servers/:id/config`、`/mcp/servers/:id/reconnect`、`/mcp/tool-policies`、`/mcp/tool-policies/:serverId/:toolName`、`/mcp/status`。
