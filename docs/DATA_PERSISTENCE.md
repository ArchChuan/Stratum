...
- 租户级表（skill_revisions、evaluation_jobs、eval_runs、evaluation_experiments、agent_execution_checkpoints、knowledge_parent_chunks、memory_active_snapshots、memory_summaries 等）由 `pkg/storage/postgres/tenant_schema.sql` 定义，`ProvisionAllTenantSchemas` 在各 `tenant_<id>` schema 中创建
