...
| # | 决策 | 结论 |
|---|---|---|
| D1 | 压缩提示词 | `agent.compaction_prompt` 由 resource 级迁到平台级；删除 `CompactionDefaultPrompt` 代码常量与 `NewLLMHistoryCompactor`/`resolveCompactionParameters` 兜底；运行时经平台参数解析，未配置即失败（对齐 `memory.*_prompt` 模式）。压缩温度/模型保持 per-agent 不变 |
...
- `internal/agent/application/agent_service.go`：
  - deps 增加 `PlatformPromptResolver`；`resolveCompactionParameters` 改为 `(ctx, resolver, cfg)`，prompt 平台解析 fail-closed，temperature/model 仍读 cfg。
  - `assembleOptions` 解析失败返回错误（执行 fail-closed）。
  - 删除 per-agent `compaction_prompt` DTO/参数/校验路径。
- `internal/agent/infrastructure/capability/history_compactor.go`：删除空 prompt 回退常量；prompt 必填。
- `pkg/constants/agent.go`：删除 `CompactionDefaultPrompt`。
- 删除 `AgentConfig.CompactionPrompt` 与 repo put/unpack、handler DTO 字段。
...
- `AgentFormSections.tsx`：删除压缩提示词字段与 `PromptDefaultViewer` 引用（保留温度/模型）。
- `useEditAgentPage.ts`、`agent.ts`：删除 `compaction_prompt`。
- `PromptDefaultViewer.tsx` 及其测试：无消费方后删除。
- 平台参数页自动渲染 `agent.compaction_prompt` / `agent.system_prompt`（schema-driven）。

## 3. 验证

- 单测：registry 迁移、prompt-defaults 空、compaction fail-closed、global suffix 解析、系统助手 DB prompt。
