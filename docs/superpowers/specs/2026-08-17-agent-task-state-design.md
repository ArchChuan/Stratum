...
- `owner` 恢复入口：`GetLatestForOwner(tenantID, agentID, userID)` —— 新会话唯一入口，跨会话推进的核心。
...
1. **generation 乐观锁**：写回 task 内容时携带读取到的 `generation`，`WHERE generation = $old`，冲突则 `ErrGenerationConflict` 重试/放弃。
...
task 内容从 Plan 映射（Plan 是目标内容的事实源）。挂点在 `BaseAgent.Execute` 结束路径（`persistChatMessages` 之后、返回 result 之前）：

```
Execute 结束，若存在 plan checkpoint →
  TaskSnapshot:
    goal            ← plan.Goal
    current_phase   ← plan 当前 phase（含状态）
    completed_steps ← plan 已 done 节点
    next_action     ← plan 下一 pending 节点
    status          ← result 正常 → active；用户明确完成 → completed；失败 → fail_count+1
  → taskRepo.Upsert + 原子 Claim
```

纯对话（无 plan）不建 task——task 是"推进目标"的载体，目标需要 plan。
...
| **task（新增）** | **新会话、跨 execution** | `GetLatestForOwner` → 注入 task 摘要 + continue 指令 | 🆕 本设计 |

新会话推进流程：

```
用户在新会话发消息 →
  task := GetLatestForOwner(tenantID, agentID, userID)
  if task.status == 'active' 且 next_action 非空:
    BuildInitMessages 注入:
      goal / current_phase / completed_steps / next_action
      + continue 指令（进入 plan continue 模式，非从零重新 plan）
    → agent 继续推进 → execution 结束更新 task + 刷新 lease
```

**不自动加载旧会话完整历史**：task 摘要是进度的压缩表示；完整历史属 checkpoint 语义（同会话断线）。
...
| **新会话恢复发现活跃 task** | task.next_action 非空 → 自动注入 continue 指令（本设计闭环） |
...
| 场景 | 处置 |
|---|---|
| task upsert/claim 失败 | 不回滚已产生的 execution 结果（用户已收到回复）；ERROR 日志 + 向上传播，下次 execution 补偿更新 |
...
- **domain**：Plan → TaskSnapshot 映射纯函数，表驱动。
- **repository**：Upsert / ClaimAtomic / GetLatestForOwner / CleanupExpired / DetachConversation——覆盖 tenantID 强制、claim 冲突、lease 过期接管、conversation 删除 detach、generation 乐观锁冲突。所有方法经 `execTenant(ctx, tenantID, fn)`，port 签名显式含 `tenantID string`。
- **application**：提取挂点（mock plan/task repo）；恢复链路（GetLatestForOwner 命中/未命中、fail closed）。
...
| `internal/agent/application/graph/` | TaskSnapshot 映射（读 plan checkpoint） |
...
1. 会话 A 激活 plan 干一半，会话 B（同 agent+user）发消息，agent 从 next_action 继续，无需用户重述目标。
2. 会话 A 删除后，会话 B 仍能恢复同一 task（detach 不级联）。
3. 两个会话同时推进同一 task，第二个会话降级只读，不互相覆盖（generation/lease 生效）。
4. 无 plan 的纯对话不产生 task 行。
5. `go vet && go test -short ./...` 通过；E2E 覆盖跨会话场景。
