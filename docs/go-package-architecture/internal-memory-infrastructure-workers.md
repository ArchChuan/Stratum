...
该包提供按租户运行的后台记忆作业，包括事实抽取、过期清理、事实 supersede 判定、实体画像重建，以及租户发现与 worker 生命周期管理。
...
flowchart TB
  ...
  tests["测试汇总<br/>extraction · GC · helpers · profile · supersede"]
...
`TenantWatcher` 周期读取租户并为新增租户构建 `WorkerSet`，移除租户时停止对应 worker。各 worker 都围绕 domain/port 执行单一后台职责，并用 `helpers.go` 的可取消等待与重启监督逻辑维持生命周期；LLM supersede adapter 复用 pipeline 的 LLM 客户端契约。
