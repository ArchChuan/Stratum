...
该包编排记忆写入、缓冲与扫描、LLM 事实抽取、实体归一化、混合召回、遗忘和按用户/Agent 清理等应用用例。
...
flowchart LR
  ...
  ext["zap"]
  tests["测试<br/>extraction · memory_service · memory_service_v2<br/>memory · message_buffer · retrieval"]
  service --> extraction
  service --> retrieval
  service --> buffer
  legacy --> types
...
`MemoryService` 是事实化记忆的新用例服务，通过仓储、向量、LLM extractor 和 superseder 端口完成抽取与召回；`MessageBuffer`/`BufferScanner` 把对话消息聚合后入抽取队列。`MemoryManager` 保留对通用 `MemoryEntry` 的管理接口。
