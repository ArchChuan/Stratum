...
- tenant-scoped port 方法签名必须包含 `tenantID string`，确保调用链能够完成租户路由
- 在 `execTenant` 外执行 `SET search_path` 时，连接释放前必须 `RESET search_path`；启动路径访问 public 表必须使用 `public.table_name` 全限定名
...
| 远程部署（真生产链路） | `.github/workflows/deploy.yml` → 镜像推阿里云 CR → Helm (`values-demo.yaml` + `values-demo-remote-http.yaml`) → K3s | CD 触发；公网 `:6879` 进入 Traefik `web2` |
...
- 修改 port 接口后必须搜索并同步所有 mock/stub 实现，确保相关包完整编译
- 实现全量删除前必须枚举关系库、向量库和消息/会话等所有关联存储，逐一确认清除范围
- pgx v5 向 JSONB 写入自定义结构体时先 `json.Marshal`，再传入编码后的字符串或字节；禁止依赖未解析 OID 的 `pgtype.JSONB` 写法

### 并发与 Context 红线

- `context.WithTimeout` 必须在每次循环迭代内独立创建和取消，禁止多个操作共享同一超时预算
- 重连或替换有状态对象时必须创建新实例并写回注册表，禁止只修改快照副本
- 多个独立 IO 操作应使用受控并发，并由 `WaitGroup` 或等价生命周期机制跟踪
- 共享 client 指针必须在锁内捕获后使用，避免 `ensureConnected` 与 `Close` 之间的 TOCTOU nil panic
- `Stop`/错误返回路径必须 cancel 后等待已启动 goroutine 退出，保持 Harness 逆序关闭语义
- context 超时后仍可能产生资源的后台任务必须排水结果并关闭成功创建的连接
...
新增测试前必须选择同域已有的高质量测试作为模板（如 `api/http/handler/skill_handler_test.go`），复用其 mock 构造方式、断言风格和边界用例组织方式。
...
| 6 | 变更预算 | 控制单次任务范围；复杂工作分批交付并记录检查点 |
...
- Layer 2（项目事实）：[`agent/project.md`](agent/project.md)
- Layer 3（模块规则）：[milvus](agent/milvus.md) · [nats](agent/nats.md) · [api](agent/api.md) · [agent](agent/agent.md) · [observability](agent/observability.md)
