...
require "$spec" 'baseURL: process\.env\.E2E_WEB_URL' 'replacement context preserves configured base URL'
require "$spec" 'viewport: testInfo\.project\.use\.viewport' 'replacement context preserves project viewport'
require "$spec" 'memberContext = await browser\.newContext\(contextOptions\)' 'member context preserves project options'
