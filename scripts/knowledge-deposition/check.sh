...
fallback="$(knowledge_shell_command bash scripts/knowledge-deposition/report.sh --client "$client" --session "$session" --task TASK --repo-root "$repo_root")"
...
command="$(knowledge_shell_command bash scripts/knowledge-deposition/report.sh --client "$client" --session "$session" --task "$task" --repo-root "$repo_root")"
...
cmp -s <(knowledge_render_markdown "$normalized") "$markdown_path" || {
  knowledge_fail "paired Markdown report does not match normalized JSON; run: $command"; exit 1;
}
