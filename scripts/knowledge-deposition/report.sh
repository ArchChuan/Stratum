...
lock_path="$repo_root/tmp/knowledge-deposition/.lock"
...
jq -r '
  def evidence:
    .evidence[] | "  - Evidence: `" + .path + (if has("anchor") then "#" + .anchor else "" end) + "`";
  [
  ...
  (if .decision == "none" then
    ...
    ([.candidates[] |
      ...
      ([evidence]) +
      ["- Exclusions/counterexamples: " + (.exclusions | join("; "))] +
      (if has("claim_group") then ["- Claim group: `" + .claim_group + "`"] else [] end) +
      (if has("consumption_purpose") then ["- Consumption purpose: " + .consumption_purpose] else [] end) +
      (if .destination == "obsidian" then [
        "- Knowledge type: `" + .knowledge_type + "`",
        "- Vault queries: " + (.vault_queries | join("; ")),
        "- Related notes: " + (.related_notes | join("; ")),
...
mv -f "$normalized_tmp" "$json_path"
normalized_tmp=''
mv -f "$markdown_tmp" "$markdown_path"
markdown_tmp=''
