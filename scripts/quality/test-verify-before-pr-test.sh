...
write_command eval eval
write_command writer write-report
write_command checker check-report
chmod 0644 "$test_dir/plan" "$test_dir/focused" "$test_dir/eval" "$test_dir/writer" "$test_dir/checker"
...
run_case() {
  ...
  TEST_VERIFY_PLAN_PATH="$test_dir/plan.json" TEST_VERIFY_PLAN_COMMAND="$test_dir/plan" \
    TEST_VERIFY_CHECKS_COMMAND="$test_dir/focused" TEST_VERIFY_EVAL_COMMAND="$test_dir/eval" \
    TEST_VERIFY_MAKE_COMMAND="$test_dir/make" \
...
run_case R1 none 'plan focused eval write-report check-report'
run_case R2 short 'plan focused eval make:e2e-system-short write-report check-report'
run_case R3 soak 'plan focused eval make:e2e-system-short make:e2e-system-soak write-report check-report'
run_case R4 release-soak \
  'plan focused eval make:e2e-system-short make:e2e-system-soak make:e2e-system-release-soak write-report check-report'
