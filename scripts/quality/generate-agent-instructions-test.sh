...
grep -Fq 'knowledge-deposition-test:' "${ROOT}/Makefile" && fail 'knowledge deposition Make target is still registered'
grep -Fq 'id: knowledge-deposition-test' "${ROOT}/.pre-commit-config.yaml" && \
  fail 'knowledge deposition pre-commit hook is still registered'
...
validate_precommit_integration() {
  local file="$1" agent_block files_regex required_path
...
assert_not_contains "${agents_content}" 'knowledge-deposition'
assert_not_contains "${claude_content}" 'knowledge-deposition'
...
! grep -Fq 'knowledge-deposition' "${ROOT}/AGENTS.md" || fail 'repository AGENTS.md contains removed gate'
! grep -Fq 'knowledge-deposition' "${ROOT}/CLAUDE.md" || fail 'repository CLAUDE.md contains removed gate'
