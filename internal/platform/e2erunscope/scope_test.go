...
func TestValidate(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "database identity", mutate: func(s *Scope) {
			s.DatabaseName = "stratum_e2e_20260730t120102z_1111111111111111"
		}},
...
func TestDatabaseURLRejectsUnsafeInputsWithoutLeakingCredentials(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "compose postgres host", base: "postgres://user:" + secret + "@postgres:5432/stratum_e2e",
			target: safeDatabaseName()},
		{name: "non E2E base database", base: "postgres://user:" + secret + "@127.0.0.1/production",
			target: safeDatabaseName()},
		{name: "test substring", base: "postgres://user:" + secret + "@127.0.0.1/contest", target: safeDatabaseName()},
		{name: "test suffix on production",
			base: "postgres://user:" + secret + "@127.0.0.1/production_test_backup", target: safeDatabaseName()},
		{name: "e2e substring on production",
			base: "postgres://user:" + secret + "@127.0.0.1/not_e2e_production", target: safeDatabaseName()},
		{name: "fragment", base: "postgres://user:" + secret + "@127.0.0.1/stratum_test#fragment",
			target: safeDatabaseName()},
		{name: "unsafe target", base: "postgres://user:" + secret + "@127.0.0.1/stratum_test",
			target: "unsafe;drop database"},
