...
func TestTenantGatewayCacheRecreatedTenantRejectsOldLoadToken(t *testing.T) {
	...
	if cache.SetIfGeneration("tenant-1", NewGateway(nil, nil, nil), nil, time.Minute, oldToken) {
		t.Fatal("old loader published after tenant recreation")
	}
	if !cache.SetIfGeneration("tenant-1", NewGateway(nil, nil, nil), nil, time.Minute, newToken) {
...
func TestTenantGatewayCacheExpiryRejectsOldLoadToken(t *testing.T) {
	cache := NewTenantGatewayCache()
	_, _, _, oldToken := cache.GetWithGeneration("tenant-1")
	if !cache.SetIfGeneration("tenant-1", NewGateway(nil, nil, nil), nil, time.Millisecond, oldToken) {
...
func TestTenantGatewayCacheSetEstablishesCurrentLoadToken(t *testing.T) {
	cache := NewTenantGatewayCache()
	cache.Set("tenant-1", NewGateway(nil, nil, nil), nil, time.Minute)
	...
	if !cache.SetIfGeneration("tenant-1", NewGateway(nil, nil, nil), nil, time.Minute, token) {
...
func TestTenantGatewayCacheReleaseOneSharedLoadKeepsOtherLoadCurrent(t *testing.T) {
	...
	if !cache.SetIfGeneration("tenant-1", NewGateway(nil, nil, nil), nil, time.Minute, secondToken) {
