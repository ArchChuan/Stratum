...
func TestRetrieveRelevantChunks(t *testing.T) {
	...
	chunks, err := rs.RetrieveRelevantChunks(context.Background(), "t1", "q", "ws1", 3, "viewer-1")
	if err != nil || len(chunks) != 2 || chunks[0] != "chunk-a" {
		t.Fatalf("chunks = %+v, %v", chunks, err)
	}
	// 极端情况：空 tenant → 明确错误。
	if _, err := rs.RetrieveRelevantChunks(context.Background(), "", "q", "ws1", 3, "viewer-1"); err == nil {
	...
	if _, err := rsBad.RetrieveRelevantChunks(context.Background(), "t1", "q", "ws1", 3, "viewer-1"); !errors.Is(err, ErrRAGDependency) {
		t.Fatalf("embed err = %v", err)
	}
	// 极端情况：Search 失败 → ErrRAGDependency。
	store.SetSearchError(errors.New("milvus down"))
	if _, err := rs.RetrieveRelevantChunks(context.Background(), "t1", "q", "ws1", 3, "viewer-1"); !errors.Is(err, ErrRAGDependency) {
	...
	if _, err := rsNil.RetrieveRelevantChunks(context.Background(), "t1", "q", "ws1", 3, "viewer-1"); err == nil {
...
func TestRAGQueryDefaultsAndCollectionName(t *testing.T) {
	...
	if got := constants.CollectionName("t1", "ws1"); !strings.Contains(got, "ws1") {
