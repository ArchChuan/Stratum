...
const (
	...
	// 审批入口。admin 天然放行。
	OpAccess
	// OpDelete 破坏性删除（workspace/document）。仅 creator 与 owner 可执行。
	OpDelete
)
...
// closed.
func (s *WorkspaceService) checkOwnership(ctx context.Context, tenantID, actorID, createdBy string, editors []string, op OwnershipOp) error {
