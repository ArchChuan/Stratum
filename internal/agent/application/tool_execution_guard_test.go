...
func TestToolExecutionGuardActiveSkillCannotUseToolOutsideSkillScope(t *testing.T) {
	...
	req := authorizedToolExecutionRequest()
	req.Actives = []port.SkillActivation{
...
func TestToolExecutionGuardActiveSkillsUnionScopeMatrix(t *testing.T) {
	cases := []struct {
		name    string
		...
		{
			name:    "no actives keeps agent scope unchanged",
			actives: nil,
			toolID:  "mcp:orders:get",
			wantOK:  true,
		},
		{
			name:    "tool in no active skill scope denied",
			actives: []port.SkillActivation{{SkillID: "skill-1", MCPToolIDs: []string{"mcp:orders:list"}}},
			toolID:  "mcp:orders:get",
			wantOK:  false,
		},
		{
			name: "tool in first active skill allowed",
			actives: []port.SkillActivation{
				{SkillID: "skill-1", MCPToolIDs: []string{"mcp:orders:get"}},
				{SkillID: "skill-2", MCPToolIDs: []string{"mcp:orders:delete"}},
			},
			toolID: "mcp:orders:get",
			wantOK: true,
		},
		{
			name: "tool in second active skill allowed",
			actives: []port.SkillActivation{
				{SkillID: "skill-1", MCPToolIDs: []string{"mcp:orders:get"}},
				{SkillID: "skill-2", MCPToolIDs: []string{"mcp:orders:delete"}},
			},
			toolID: "mcp:orders:delete",
			wantOK: true,
		},
		{
			name: "tool in both actives allowed",
			actives: []port.SkillActivation{
				{SkillID: "skill-1", MCPToolIDs: []string{"mcp:orders:get", "mcp:orders:delete"}},
				{SkillID: "skill-2", MCPToolIDs: []string{"mcp:orders:delete"}},
			},
			toolID: "mcp:orders:delete",
			wantOK: true,
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			req := authorizedToolExecutionRequest()
			req.Actives = tc.actives
			req.Tool.Name = tc.toolID
			req.Tool.CapabilityID = tc.toolID
			// agent 边界保持 AND：AgentToolIDs 同步放行目标工具，只让 skill 并集决定成败。
			req.AgentToolIDs = append([]string{"mcp:orders:get"}, tc.toolID)
			...
			require.ErrorIs(t, err, ErrToolAuthorizationDenied)
			require.Contains(t, err.Error(), string(domain.ToolReasonSkillScopeExceeded))
			require.Zero(t, executor.calls)
		})
	}
}
