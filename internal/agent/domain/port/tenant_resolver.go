...
type TenantCapabilityResolver interface {
	// Resolve returns the per-tenant CapabilityGateway. Returns (nil, false)
	// when no tenant-specific keys are configured.
	Resolve(ctx context.Context, tenantID string) (CapabilityGateway, bool)
