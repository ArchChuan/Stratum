...
func (r *PgSkillVersionRepo) GetDraftVersion(ctx context.Context, skillID string) (domain.SkillVersion, bool, error) {
	...
	err := tenantdb.ExecTenant(ctx, r.pool, func(ctx context.Context, tx pgx.Tx) error {
		row := tx.QueryRow(ctx,
			`SELECT id, skill_id, COALESCE(version_no, 0), status, capability, tool_contract, implementation,
...
func (r *PgSkillVersionRepo) GetActiveVersion(ctx context.Context, skillID string) (domain.SkillVersion, bool, error) {
	...
	err := tenantdb.ExecTenant(ctx, r.pool, func(ctx context.Context, tx pgx.Tx) error {
		row := tx.QueryRow(ctx,
			`SELECT v.id, v.skill_id, COALESCE(v.version_no, 0), v.status, v.capability, v.tool_contract,
...
func (r *PgSkillVersionRepo) UpdateDraftCapability(ctx context.Context, skillID string, capability domain.Capability) (domain.SkillVersion, error) {
	var version domain.SkillVersion
	err := tenantdb.ExecTenant(ctx, r.pool, func(ctx context.Context, tx pgx.Tx) error {
		...
		row := tx.QueryRow(ctx,
			`UPDATE skill_versions
			 SET capability=$2, updated_at=NOW()
			 WHERE skill_id=$1 AND status='draft'
			 RETURNING id, skill_id, COALESCE(version_no, 0), status, capability, tool_contract, implementation,
			           test_baseline, publish_checks`,
			skillID, string(capabilityJSON),
		)
		v, err := scanSkillVersion(row)
		if err != nil {
			return err
...
func (r *PgSkillVersionRepo) UpdateDraftContract(ctx context.Context, skillID string, contract domain.ToolContract) (domain.SkillVersion, error) {
	payload, err := json.Marshal(contract)
	if err != nil {
		return domain.SkillVersion{}, fmt.Errorf("skill_version_repo: marshal contract: %w", err)
	}
	return r.updateDraftJSON(ctx, skillID, "tool_contract", string(payload))
}

func (r *PgSkillVersionRepo) UpdateDraftImplementation(ctx context.Context, skillID string, implementation domain.Implementation) (domain.SkillVersion, error) {
	...
	return r.updateDraftJSON(ctx, skillID, "implementation", string(payload))
}

func (r *PgSkillVersionRepo) updateDraftJSON(ctx context.Context, skillID, column, payload string) (domain.SkillVersion, error) {
	var version domain.SkillVersion
	err := tenantdb.ExecTenant(ctx, r.pool, func(ctx context.Context, tx pgx.Tx) error {
		row := tx.QueryRow(ctx,
...
func insertSkillVersion(ctx context.Context, tx pgx.Tx, version domain.SkillVersion) error {
	...
	_, err = tx.Exec(ctx,
		`INSERT INTO skill_versions
		 (id, skill_id, version_no, status, capability, tool_contract, implementation, test_baseline, publish_checks)
		 VALUES ($1, $2, NULLIF($3, 0), $4, $5, $6, $7, $8, $9)`,
		version.ID, version.SkillID, version.VersionNo, string(version.Status), string(capabilityJSON),
		string(contractJSON), string(implementationJSON), string(baselineJSON), string(checksJSON),
	)
	return err
}
...
func scanSkillVersion(row versionScanner) (domain.SkillVersion, error) {
	var version domain.SkillVersion
	var status string
	var capabilityJSON, contractJSON, implementationJSON, baselineJSON, checksJSON []byte
	if err := row.Scan(
		&version.ID,
		&version.SkillID,
		&version.VersionNo,
