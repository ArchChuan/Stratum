...
func NewDBSkillAdapter(pool *pgxpool.Pool, gateway *llmgateway.Gateway, logger *zap.Logger, executor *code.CodeExecutor) *DBSkillAdapter {
...
func buildSkill(id, name, desc, skillType string, cfg map[string]any, gw *llmgateway.Gateway, logger *zap.Logger, executor *code.CodeExecutor) (domain.Skill, error) {
	switch skillType {
	case "http":
