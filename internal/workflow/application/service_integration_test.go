...
func TestStage1CConcurrentStartAndPublishAreAtomic(t *testing.T) {
	...
	runs := application.NewRunServiceWithRegistry(store, store, integrationRegistry{}, uuid.NewString, zap.NewNop())
...
func TestStage1BRealPostgresIndependentWorkerRunsDiamondAndPersistsEvents(t *testing.T) {
	...
	events, err := runs.Events(tenantCtx, tenantID, run.ID, adminActor(), 0, 100)
	require.NoError(t, err)
...
func TestStage1CPauseResumeAcrossWorkersUsesPostgresCheckpoint(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = controls.Pause(tenantCtx, tenantID, run.ID, run.Generation, adminActor(), "maintenance")
	require.NoError(t, err)
	...
	resumed, err := controls.Resume(tenantCtx, tenantID, run.ID, paused.Generation, adminActor())
	require.NoError(t, err)
