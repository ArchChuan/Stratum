...
var (
	...
	ErrExperimentStableNotPublished    = errors.New("experiment stable revision must be published")
	ErrExperimentInvalidCandidate      = errors.New("experiment canary must be an active optimization candidate")
	ErrExperimentSuiteNotPublished     = errors.New("experiment suite revision must be published for the resource kind")
	ErrExperimentOfflineRunRequired    = errors.New("experiment candidate requires a passed offline evaluation run")
...
func (c CandidateCommand) Validate() error {
	if strings.TrimSpace(c.ActorID) == "" || c.ActorType != ActorTypeAdmin || strings.TrimSpace(c.Reason) == "" ||
		strings.TrimSpace(c.IdempotencyKey) == "" || c.ExpectedStateVersion < 1 {
		return ErrInvalidCandidateCommand
	}
	return nil
}
