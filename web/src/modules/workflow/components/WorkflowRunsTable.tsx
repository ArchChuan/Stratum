...
export const WorkflowRunsTable = ({ runs, loading, pagination, emptyText, onOpen, onPageChange }: {
  ...
  const columns: ColumnsType<WorkflowRunSummary> = [
    { title: '工作流', dataIndex: 'name', key: 'name', ellipsis: true, render: (value, run) => <div><Text strong>{value || run.definition_id}</Text><div><Text type="secondary">版本 {run.version}</Text></div></div> },
  ...
  return <ResponsiveDataView
    ...
    renderMobileItem={(run) => <Card><Space direction="vertical"><Text strong>{run.name || run.definition_id}</Text><Tag color={statusColors[run.status]}>{statusLabels[run.status]}</Tag><Text type="secondary">{formatTime(run.started_at)}</Text><Button aria-label="查看运行" onClick={() => onOpen(run)}>查看详情</Button></Space></Card>}
  />;
};
