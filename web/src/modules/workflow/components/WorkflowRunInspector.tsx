...
export const WorkflowRunInspector = ({ state, nodeId }: { state: WorkflowRunState; nodeId?: string }) => {
  ...
  return <aside aria-label="运行步骤详情" className="workflow-run-inspector">
    ...
    <Collapse items={attempts.map((attempt) => ({ key: attempt.id, label: `第 ${attempt.attempt_no} 次尝试 · ${attempt.status}`, children: <>
      {attempt.input ? <Paragraph><Text type="secondary">输入：</Text>{attempt.input}</Paragraph> : null}
      <Paragraph><Text type="secondary">输出：</Text>{attempt.output_summary || '暂无摘要'}</Paragraph>
      {attempt.error_message && <Paragraph type="danger">{attempt.error_message}</Paragraph>}
    </> }))} />
