import { Alert, Button, Collapse, Drawer, Flex, Modal, Space, Typography } from 'antd';

import type { CandidateSummary } from '../model/evaluation';

import { drawerWidth, SafeValue, StatusTag } from './evaluationView';
export const CandidateDrawer = ({ candidate, open, onClose, canManage, onReject, onEvaluate, isMobile }: {
  candidate: CandidateSummary | null; open: boolean; onClose: () => void; canManage: boolean;
  onReject: (candidate: CandidateSummary) => void;
  onEvaluate?: (candidate: CandidateSummary) => void; isMobile?: boolean;
}) => {
  ...
  const canReject = canManage && candidate?.status === 'proposed';
  return <Drawer title="候选版本" open={open} onClose={onClose} width={drawerWidth(isMobile)} destroyOnHidden
      extra={canReject ? <Space><Button onClick={() => candidate && onEvaluate?.(candidate)}>运行离线评测</Button>
        <Button danger onClick={reject}>拒绝候选</Button></Space> : null}>
      {candidate && <>
        ...
        {canReject && <><Typography.Title level={5}>管理员决定</Typography.Title>
